...
| **Сроки/SLA + уведомления** | ✅ | `deadline_at` в кэше, бейдж срочности; TG-рассылка о дедлайнах (env-gated, выключена) |
...
- **Обучение ИИ**: `training_samples` копится (закрытие + бэкафилл `/training/backfill`) — следующий шаг few-shot retrieval / fine-tune; метрика совпадения ответа ИИ с оператором ещё не считается.
- **TG-уведомления о сроках**: код готов (env-gated), на проде НЕ активированы — нужно задать `TELEGRAM_ALERT_CHAT_ID` и перезапустить.
