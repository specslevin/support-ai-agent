...
### Список заявок
- Фильтрация по статусу, компании, поиск по теме
...
- Кнопка «🗺 Карта и графики» в шапке деталей → панель выезжает влево.
...
- Чекбоксы в строках + «выбрать всё» на странице.
...
| PATCH | `issues/{id}` | Обновить заявку (используется для `assignee_id`) |
