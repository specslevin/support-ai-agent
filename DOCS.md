...
**Последнее обновление:** 2026-06-27 (сессия 12)  
**Статус:** Production — дашборд + ИИ-автоанализ + вложения/OCR + карта/графики + массовое редактирование + разбор «общих» заявок по форматам вложений + сроки/SLA
...
- Таблица: гос.номер / дата / ПЛ / **ГЛОНАСС заявл.** (от клиента) / **По факту** (реальный трек) / вердикт + 🗺 трек каждого ТС. Спецтехника помечается бейджем (без км-вердикта).
