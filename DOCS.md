...
**Статус:** Production — дашборд + ИИ-автоанализ + вложения/OCR + карта/графики + массовое редактирование + разбор «общих» заявок по форматам вложений + сроки/SLA + петля обратной связи ИИ + копирование монтажнику + личные шаблоны
...
- Эндпоинты: `POST /issues/{id}/ai_feedback`, `GET /issues/{id}/ai_feedback` (последняя), `GET /issues/ai_feedback/list[?rating=]`. Экран «Оценки ИИ» (`AiFeedbackReview.tsx`) — хорошо разобрано / с ошибками; основа для роста few-shot и точечной правки промпта/правил.
