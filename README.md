...
- **Дашборд заявок**: список с фильтрами (№ заявки, тема, статус, ответственный, компания — комбинируются) и пагинацией (запоминается), личные сохранённые фильтры (CRUD, подсветка активного, «Сбросить»), детали из Okdesk, комментарии, смена статуса/типа/ответственного.
...
- **Разбор «общих» заявок по форматам вложений**: мульти-акт PDF / список ТС в теле / сводная таблица / XLSX «Группировка» (по парсеру на формат) + формат-профили ПО (дочерние Россети Волга). Таблица вердиктов по каждому ТС («ГЛОНАСС заявл.» vs «По факту», бейдж спецтехники), редактируемый вердикт-по-дате, 🗺 трек каждого ТС, один общий ответ и создание вложенных заявок.
- **Детектор «ложного wait»**: клиент подтвердил питание/просит удалённую диагностику → предупреждение, заявка не зависает на исчерпанном шаблоне.
- **Кэш анализа**: результаты автоанализа и разбора сохраняются — при открытии показываются без повторного прогона ИИ (экономия токенов), кнопка «Обновить» для форс-прогона.
- **Задел для обучения**: каждое решение пишется в `training_samples` (телеметрия + ответ оператора); few-shot retrieval подмешивает похожие решённые кейсы и шаблоны категории в промпт.
- **Петля обратной связи ИИ**: оператор оценивает разбор (good/bad + тип ошибки + комментарий) — экран «Оценки ИИ» (`/issues/ai_feedback`).
- **Копирование монтажнику**: готовые тексты КАЛЕНДАРЬ/МЕССЕНДЖЕР по заявке (телефон/контакт/адрес/ТС/«не в сети с …») в один клик (`/installer_export`).
- **Редактирование параметров заявки**: Местоположение техники / Контактное лицо / Номер телефона прямо из карточки (`POST /parameters`) — заполняет обязательные поля для перевода «В работе».
- **Личные шаблоны**: у каждого оператора своя база шаблонов (`user_id`) поверх общих; сортировка по использованию, дедуп, динамические `[вчера]`/`[завтра]`/`[сегодня]`.
- **Telegram-алерты о сроках**: рассылка о приближающихся дедлайнах включена (`TELEGRAM_ALERT_CHAT_ID`).
- **Роли и управление пользователями**: admin создаёт операторов и помечает их бейджем «можно авто»; демо-витрина только для чтения. Авторизация с fail-fast при пустом/дефолтном `AUTH_SECRET` в production.
...
| POST/GET | `/api/v1/issues/{id}/automate` | POST — ИИ-автоанализ + черновик; GET — кэш (без прогона) |
| GET | `/api/v1/issues/{id}/track[?plate=&date=]` | Трек + телеметрия; plate/date — трек конкретного ТС |
...
| POST/GET | `/api/v1/issues/training/backfill\|stats` | Бэкафилл TrainingSample из решённых + статистика базы знаний |
| GET | `/api/v1/issues/{id}/installer_export` | Готовые тексты «монтажнику» (КАЛЕНДАРЬ/МЕССЕНДЖЕР), read-only |
| POST | `/api/v1/issues/{id}/parameters` | Обновить кастом-параметры (address/contact_person/tel_person) |
| POST/GET | `/api/v1/issues/{id}/ai_feedback` | Оценка ИИ-разбора оператором / последняя оценка |
