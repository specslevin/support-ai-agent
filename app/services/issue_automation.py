...
_FORMAT_PROFILES: list[FormatProfile] = [
    ...
    FormatProfile(
        # Общефилиальный ключ для ОСТАЛЬНЫХ Ульяновских ПО (Южное,
        # Димитровградское, Барышское): раньше они не попадали ни в один профиль,
        # т.к. ключ был «Ульяновское ПО». ФОРМАТ НЕ ПОДТВЕРЖДЁН ПРИМЕРАМИ — все
        # известные заявки (65976, 65975, 65974, 65973) без вложений, как выглядит
        # их акт, неизвестно. Поэтому профиль НЕЙТРАЛЬНЫЙ: чужую (табличную)
        # стратегию Ульяновского ПО им НЕ приписываем, поля data_location/
        # cardinality/attachment_format здесь заглушки, значим только hint.
        # Как появятся заявки с вложениями — завести отдельные ключи по ПО.
        "Ульяновские РС", "subject_or_attachments", "single", "none",
        "Формат заявок этого ПО НЕ подтверждён примерами — специальной стратегии "
        "нет. Разбирай общей эвристикой: гос.номер и дату ищи и в теме, и в "
        "тексте, и во вложениях; не предполагай ни таблицу, ни акт-на-объект."),
    FormatProfile(
...
_DECLARED_KEY_FULL = r"(?:глонас\w*|ссм|смс|галактик\w*|в\s*ПК|в\s*систем\w*)"
...
def _clamp_year_to_created(iso: str | None, created: _dt.date | None) -> str | None:
    ...
    try:
        ...
        return iso
    if (d - created).days <= _FAULT_FUTURE_SLACK_DAYS:
        return iso
