...
def _mileage_matches(t: "TelemetryFacts", sheet: float | None) -> bool:
    ...
    Недостоверная цифра (накрученная телепортами) ничего не подтверждает —
    сравнение по ней не проводим вовсе (класс 10).
    """
...
def _jamming_conflicts_with_mileage(t: "TelemetryFacts", sheet: float | None) -> bool:
    ...
    return abs(sheet - system) <= tol or system - sheet > tol
...
def _act_name_body_differ(name: str | None, text: str | None) -> bool:
    ...
    (в теле он может повторяться в шапке и в подписи).
    """
...
class IssueAutomationService:
    ...
    def _heuristic_category(self, parsed: ParsedIssue, f: TelemetryFacts) -> str:
        ...
        if "jamming" in f.flags:
            ...
            # неверный ответ (сессия C4, классы 7, 8).
            return ("Проверить"
