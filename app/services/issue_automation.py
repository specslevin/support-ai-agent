...
def _sync_labels(item: dict[str, Any]) -> None:
    ...
    if not hint or verdict == hint:
        return
    if verdict == "Проверить" and hint in _CAUSAL_VERDICTS:
        item["heuristic_category"] = "Проверить"
...
_PLATE_RE = re.compile(
    ...
    # Страж нужен И СЛИТНОЙ ветке региона: без него «НВ29.07.26г» (66191)
    # откусывало ДЕНЬ даты как код региона и давало выдуманный «Н718НВ29» —
    # формально валидный номер, который потому и не помечался подозрительным
    # (сессия C3, класс 1). Пустая альтернатива идёт ПОСЛЕДНЕЙ и стража не
    # имеет: когда регион взять нельзя, номер обязан извлечься без него.
    rf"(?:[ \t]{{1,2}}\d{{2,3}}(?!\d)(?![.,/-]\d{{1,3}}(?:[.,/-]|\s*г\b|$))|\d{{1,3}}(?!\d)(?![.,/-]\d{{1,3}}(?:[.,/-]|\s*г\b|$))|)"
...
_PLATE_STD_RE = re.compile(
    ...
    # Пустая альтернатива последняя и без стража: когда регион взять нельзя,
    # номер обязан извлечься без него. Диапазон `\d{1,3}`, а не `\d{2,3}`:
    # односимвольный «регион» — это разорванный OCR-ом настоящий («Н256РМ7» из
    # «Н256РМ73», 65771), его СОХРАНЯЕМ, чтобы plate_format_suspect пометил
    # номер битым (правка П2).
    rf"(?:[ \t]{{1,2}}\d{{2,3}}(?!\d)(?![.,/-]\d{{1,3}}(?:[.,/-]|\s*г\b|$))|\d{{1,3}}(?!\d)(?![.,/-]\d{{1,3}}(?:[.,/-]|\s*г\b|$))|)",
    re.I,
)
...
class IssueAutomationService:
    ...
    def _heuristic_category(self, parsed: ParsedIssue, f: TelemetryFacts) -> str:
        ...
        if "jamming" in f.flags:
            ...
            # снимает «Глушение» и при прострелах трека.
            return ("Проверить"
    ...
    async def analyze_batch(self, issue_external_id: int, attachments: list[Any],
        ...
        if not extractable:
            ...
            body_rows = _parse_body_vehicles(
                ...
                dups_out=subj_dups)  # (plate,date,sheet,glonass)
    ...
    def _verdict_from_facts(t: TelemetryFacts, sheet: float | None,
        ...
        if "jamming" in t.flags:
            ...
            return "Проверить" if _jamming_conflicts_with_mileage(t, sheet) else "Глушение"
    ...
    def client_comment_faults(
        ...
        """Жалобы клиента из переписки: ``(дата, ПЛ, система, м/ч, конец интервала)``.

        Конец интервала — пятое поле: жалоба в комментарии тоже бывает интервалом
        ...
        for raw_date, text in client_comments or []:
            ...
            _decl, _open = _split_ranges(text, p.date, p.date_to)
            out.append((p.date, p.sheet_mileage_km, p.declared_system_km,
                        p.engine_hours,
                        _row_date_to(p.date, p.sheet_mileage_km,
                                     p.declared_system_km, _decl, _open)))
        return out
    ...
    async def _append_client_comment_rows(
        ...
        for date, sheet, declared, hours, date_to in self.client_comment_faults(
                client_comments):
            key = (_plate_dedup_key(plate), date)
            if key in have:
                continue
            ...
            try:
                extra.append(await self._analyze_object(
                    ...
                    declared=declared, engine_hours=hours, date_to=date_to))
