...
export interface BatchObject {
  ...
  /**
   * Окно смены, о котором писал клиент («08:00-20:00», «20:00-08:00»). Пробег и
   * телеметрия строки считаются за СУТКИ — окно лишь показывает, о какой смене
   * речь, и различает две строки одного ТС за один день (дневная и ночная).
   */
